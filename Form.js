...
function createField(form, field) {
  ...
  function createFormInModal(courseOptions, styles, logo, contact) {
    const formContent = `
        <div id="formContainer" class="${styles}">
            <!-- Your form HTML content here -->
            <form id="studentDetailsForm">
                <!-- Form fields go here -->
            </form>
        </div>
    `;
    createModal(formContent);
    initializeForm(); // Call initializeForm after creating the modal
}
  // Modify createFormButton to open modal instead of directly initializing form
  function createFormButton() {
    console.log('Creating form button...');
    const button = document.createElement('button');
    button.textContent = 'Open Form';
    button.addEventListener('click', () => {
        console.log('Button clicked. Creating form in modal...');
        createFormInModal('courseOptions', 'styles', 'logo', 'contact');
        button.style.display = 'none'; 
    });
    document.body.appendChild(button);
    console.log('Form button created.');
}
