...
function createSelectField(form, label, inputId, inputName, options) {
  ...
  JSON.parse(options).forEach(optionValue => {
    const option = document.createElement('option');
...
function BasicinitializeForm() {
  ...
  try {
    if (JSON.parse(path).includes(currentPath)) {
      ...
      createFormBasic(formContainer, courses, styles, logo, contact);
    } else {
...
function createFormBasic(container, courseOptions, styles, logo, contact) {
  const formContainer = document.createElement('div');
  formContainer.id = 'formContainer';
  formContainer.classList.add('form-container');
